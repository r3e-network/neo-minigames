...
# scripts/ sits directly under the repo root here; in the platform repo this
# script lived at deploy/scripts/ and needed one more level.
...
"$NCCS" contracts/TarotOracleMockFixture/TarotOracleMockFixture.csproj \
  --checked --optimize=All --output contracts/build/
node --test \
  scripts/lib/tarot_vrf_contract_artifact.test.mjs \
  scripts/lib/tarot_vrf_morpheus_abi.test.mjs
