...
export function createReactAppConfig(appDir: string, options: ReactAppConfigOptions = {}) {
  ...
  const sharedDir = path.dirname(require.resolve("@r3e-network/neo-miniapp-shared/package.json"));
  const frameworkDir = path.dirname(require.resolve("@r3e-network/neo-miniapp-framework/package.json"));
